...
export function convertPathOperationCallToFunction (
  ...
  const resultTypeName = `Promise<${capitalizeFirstLetter(op.operationId)}Result>`
  ...
  func.lines += "throw new Error(`Service call failed to ${props.baseUrl}\n${e.name}: ${e.message}.`)\n";
  func.lines += "}/n";
...
function generateUrlClause (pathUrl: string, path: OpenApiSpecPath) {
  ...
  for (const queryParam of path.parameters) {
    if (queryParam.in === 'query') {
      const queryAssignment = `"${queryParam.name}=" + props.${queryParam.name}.toString()`
...
function generateValidationClause (op: OpenApiSpecPathOperation) {
  ...
  block += "} else {\n";
  block += "const statusLine = `Service call rejected with status code ${response.status}.`\n";
  block += "const errorTextLine = await response.text();\n";
  block += "const urlLine = `Url: ${url}`\n";

  if (typeof op.requestBody === "undefined") {
    block += "const bodyLine = `Body: ${JSON.stringify(props.body, null, 2)}`\n";
  } else {
    block += "const bodyLine = \"\";\n";
  }

  block += "throw new Error(`${statusLine}\n${errorTextLine}\n${urlLine}\n${bodyLine}`)";
...
function generateResponseClause (op: OpenApiSpecPathOperation) {
  ...
  if (typeof response.content?.["application/json"].schema !== 'undefined') {
    const resultType = convertPathSchemaToTypeName(response.content["application/json"].schema)
